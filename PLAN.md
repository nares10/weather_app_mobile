...
| Tests | Jest (`jest-expo`) for pure logic and the API layer with a mocked `fetch`: weather codes + legend, formatting, address, parseForecast, fetchJson, searchCities. Tests live in `__tests__/` folders and run in New York time (`jest.globalSetup.js`) to catch timezone bugs. Run `npm test`. |
